...
function ExamMap() {
  ...
  useEffect(() => {}, [examSessions]);

  const { isLoaded } = useJsApiLoader({
    id: "google-map-script",
    googleMapsApiKey: process.env.REACT_APP_GOOGLE_MAPS_API_KEY, //Enter your API key here or in an environment file
  });
        ...
        {examSessions
          .filter(
            (session) =>
              session.latitude &&
              session.longitude &&
              !isNaN(parseFloat(session.latitude)) &&
              !isNaN(parseFloat(session.longitude))
          )
          .map((session) => {
            const latAndLon = {
